...
export function MarketAnalysisDisplay({ analysis }: MarketAnalysisProps) {
  const { marketTitle, marketDescriptionSummary, marketOptions, reasons, whaleOpinions, opposingViews, triggers, metadata } = analysis;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4">
      <div className="p-4 bg-white rounded-lg border border-gray-200">
        ...
        {marketDescriptionSummary && (
          <div className="mb-3 p-3 bg-gray-50 rounded border border-gray-100">
            <h2 className="text-sm font-semibold text-gray-700 mb-2">解決条件（要約）</h2>
            <p className="text-sm text-gray-600 whitespace-pre-wrap">{marketDescriptionSummary}</p>
          </div>
        )}
        <a
            ...
            {whaleOpinions.map((opinion, index) => (
              <div key={index} className="p-3 bg-gray-50 rounded border border-gray-100">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm text-gray-500">({opinion.position})</span>
                </div>
        ...
        <div className="p-4 bg-white rounded-lg border border-gray-200">
          ...
          <ul className="list-disc list-inside space-y-1">
            {opposingViews.map((view, index) => (
              <li key={index} className="text-gray-700">{view}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-4 text-sm text-gray-500 p-4 bg-white rounded-lg border border-gray-200">
        <span>分析: {new Date(metadata.analyzedAt).toLocaleString("ja-JP")}</span>
